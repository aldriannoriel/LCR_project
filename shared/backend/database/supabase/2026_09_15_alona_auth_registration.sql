...
CREATE TABLE IF NOT EXISTS public.alona_rider_locations (
    ...
    created_at TIMESTAMPTZ NOT NULL DEFAULT now(),
    CONSTRAINT alona_user_rider_locations_unique UNIQUE (rider_id, psgc_code)
);

CREATE INDEX IF NOT EXISTS alona_users_approval_status_idx ON public.alona_users(approval_status);
CREATE INDEX IF NOT EXISTS alona_users_role_idx ON public.alona_users(role);
CREATE UNIQUE INDEX IF NOT EXISTS alona_users_active_email_idx ON public.alona_users(lower(email)) WHERE approval_status IN ('PENDING', 'APPROVED');
...
CREATE POLICY alona_users_admin_all ON public.alona_users FOR ALL TO authenticated
USING ((auth.jwt() -> 'app_metadata' ->> 'role') = 'admin')
WITH CHECK ((auth.jwt() -> 'app_metadata' ->> 'role') = 'admin');

DROP POLICY IF EXISTS alona_rider_details_admin_all ON public.alona_rider_details;
CREATE POLICY alona_rider_details_admin_all ON public.alona_rider_details FOR ALL TO authenticated
USING ((auth.jwt() -> 'app_metadata' ->> 'role') = 'admin')
WITH CHECK ((auth.jwt() -> 'app_metadata' ->> 'role') = 'admin');

DROP POLICY IF EXISTS alona_staff_details_admin_all ON public.alona_staff_details;
CREATE POLICY alona_staff_details_admin_all ON public.alona_staff_details FOR ALL TO authenticated
USING ((auth.jwt() -> 'app_metadata' ->> 'role') = 'admin')
WITH CHECK ((auth.jwt() -> 'app_metadata' ->> 'role') = 'admin');

DROP POLICY IF EXISTS alona_rider_locations_admin_all ON public.alona_rider_locations;
CREATE POLICY alona_rider_locations_admin_all ON public.alona_rider_locations FOR ALL TO authenticated
USING ((auth.jwt() -> 'app_metadata' ->> 'role') = 'admin')
WITH CHECK ((auth.jwt() -> 'app_metadata' ->> 'role') = 'admin');